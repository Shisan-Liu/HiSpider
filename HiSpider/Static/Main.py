...
import time
from Static.myRequests import MyRequest as RequestController
from Static.analyze import mainAnalyze as AnalyzeController
from Static.log import Log as LogController
from Static.save import Save as SaveController
...
class HiSpider:
    def __init__(self):
        self.localLog = LogController()
        self.localSave = SaveController(self.localLog)
        self.localRequest = RequestController()
        self.localAnalyze = AnalyzeController()
    ...
    def ReadModel(self):
        #read conf file, edit in softwere
        print()

    def seturl(self,url,modelname,format):
        # 缺少验证URL
        # 获取HTML代码
        self.TargetUrl=url
        self.Html = self.localRequest.gethtml(self.TargetUrl)

    def StartAnalyze(self):
        # 分析HTML
        self.localAnalyze.start(self.Html)
        diclist = self.localAnalyze.diclist
        #print(diclist)
    
    def SaveModel(self,modelname,format):
        # 保存DIC
        ...
        self.localSave.write(foldername,filename,diclist,filetype)
